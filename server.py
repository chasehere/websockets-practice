...
class SomeServerProtocol(WebSocketServerProtocol):
  ...
  def onConnect(self, request):
    print("some request connect {}".format(request))

  def onMessage(self, payload, isBinary):
    """
    Message sent from client, communicate this message to its
...
class ChatRouletteFactory(WebSocketServerFactory):
  ...
  def register(self, client):
    """
    Add client ot list of managed connections.
    """
  ...
  def findPartner(self, client):
    """
    Find chat partner for client.  Check if any of tracked clients
    is idle.  If there is no idle client just exit quietly.  If there
    is available partner assign him/her to our client.
    """
  ...
  def communicate(self, client, payload, isBinary):
    ...
    if not c["partner"]:
      c["object"].sendMessage("Sorry you do not have a partner yet, check back later.")
    else:
